...
#        return tmp
        ...
        meanFile = method(input=fileName, output=self.tmpDir+fn+str(pid)+self.analysisOptions['cdomean'])
        selstr = ','.join(map(str, range(startyear, endyear+1)))
        seldate = cdo.seltimestep(selstr, input=meanFile,
                                  output=fileNameOut+'_selYear_'+str(startyear)+'-'+str(endyear)+str(pid)+self.getRandomStr())
        tmp = cdo.timmean(input=seldate, output=seldate+'Timmean')
        return tmp
    ...
    def getAnomalies(self, ensMeanList, crossMean):
        ...
        for year in self.decadals:
            anomalies[year] = cdo.sub(input=' '.join([ensMeanList[year], crossMean[year]]),
                                      output=ensMeanList[year]+'_ANOMALIE')
        return anomalies 
    ...
    def createConstantFile(self, gridFile, flag=''):
        ...
        if self.fieldmean:
            ...
            tmp_const = cdo.setrtoc('-1e99,1e99,1', input=self.observationRemapped[self.decadals[0]],
                                    output=self.tmpDir+flag+'constantField_tmp.nc',
                                    options='-f nc')
    ...
    def getEnsembleStd(self, ensList):
        ...
        return cdo.sqrt(input=cdo.ensmean(input=' '.join(result.values()),
                                          output=result.values()[0]+'ensmean_afgst'),
                        output=result.values()[0]+'AVGSTD')
...
#        detrended = cdo.mergetime(' '.join(detrended_list), output=fn+'_detrended')    
    ...
    def getMissingMaskForFieldMean(self):
        ...
        yearly_obs = dict()
        for year in self.decadals:
            yearly_obs[year] = cdo.yearmean(input=self.observationRemapped[year],
                                            output=self.tmpDir+self.extractFilename(self.observationRemapped[year]+'_ymean'))
