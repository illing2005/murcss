...
class MsssBootstrap(Msss):
    ...
    def _prepareBootstrap(self, year):
        ...
        ensmeanProject1 = cdo.ensmean(input=' '.join(tmpList2),
                                      output=tmpOut+str(year)+self.experiment1+self.project1+'_'+self.model1+'_'+self.product1+self.getRandomStr()+'.nc')
        ...
        self.makeFolder(tmpOut)
        ensmeanProject2 = cdo.ensmean(input=' '.join(tmpList2),
                                      output=tmpOut+str(year)+self.experiment2+self.project2+'_'+self.model2+'_'+self.product2+self.getRandomStr()+'.nc')
    ...
    def prepareBootstrap(self):
        ...
        if self.zonalmean:
            ...
            for year in self.decadals:
                ...
                #tmpList.append()
                ensMeanProject2Dict[year] = cdo.sellevel(','.join(level_intersection), input=tmp_file,
                                                         output=tmp_file+'sellevel')
    ...
    def bootstrapGoddard(self, outputFolder='./tmp/'):
        ...
        for year in self.decadals:
            try:
                ...
                bootstrapResultM1[year] = newPoolList1[yearToSelect]
                bootstrapResultM2[year] = newPoolList2[yearToSelect]
                ...
                for i in range(1, 6):
                    if yearToSelect+i < max(self.decadals) and year+i <= max(self.decadals):
                        ...
                        if yearToSelect+i in self.decadals: 
                            bootstrapResultM1[year+i] = newPoolList1[yearToSelect+i]
                            bootstrapResultM2[year+i] = newPoolList2[yearToSelect+i]
