...
class FindFilesAbstract(ToolAbstract):
    ...
    def checkGrid(self, fList, model):
        ...
        # single process, because multiproccessing caused memory problems
        result = list()
        for fn in fList:
    ...
    def _selectLevel(self, files):
        ...
        try:
            return cdo.sellevel(self.level, input=files,
                                output=self.tmpDir+self.getFilename(files)+'_'+str(self.level)+'.nc')
        except:
