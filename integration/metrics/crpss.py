...
class CrpssError(Exception):
    pass


class NotEnoughEnsemblemembersFound(CrpssError):
    pass


class VariableName(CrpssError):
    pass
...
class Crpss(MetricAbstract):
    ...
    def __init__(self,
                 ...
                 months=None):
        """
        Constructor. Set parameters and calls parent constructor (metricAbtract)
        """
        ...
        listOfAttributes = ['decadals', 'variable',
                            ...
                            'observation_ensemble', 'bootstrapSwitch', 'bootstrap_number',  # for bootstrapping
                            'observation_type', 'colormap', 'timeFreq', 'basic_output', 'input_part']
        for attr in listOfAttributes:
            setattr(self, attr, locals()[attr])

        if ensemblemembers is not None and ensemblemembers != '*':
    ...
    def prepareInput(self):
        ...
        self.findFiles.observation = self.observation
        
        self.inputDict = dict()
        self.observationDict = dict()
        print 'Searching Files' 
        countYears = len(self.decadals)
        poolArgs = self.getPoolArgs(countYears, self.findFiles, list(self.decadals), self.project,
                                    self.model,self.variable, self.timeFreq, self.product1, self.ensemblemembers,
                                    self.institute1, self.experiment, self.maxLeadtime, self.minLeadtime, 'getFiles')
        ...
        if self.zonalmean:
            ...
            level_intersection = self.getLevelIntersection(self.observationRemapped[self.decadals[0]],
                                                           self.inputRemapped[self.decadals[0]][0])
            ...
            for year in self.decadals:
                ...
                self.inputRemapped[year] = self.multiProcessCdo(self.inputRemapped[year], 'sellevel',
    ...
    def analyze(self):
        ...
        for yearRange in temporalSmoothing:
            ...
            for year in self.decadals:
                tempSmoothedObs[year] = cdo.sub(input=' '.join([tempSmoothedObs[year], tmp2]),
                                                output=tempSmoothedObs[year]+'meanAgain'+str(startyear)+str(endyear))
            ...
            ensemblespreadscore = cdo.div(input=' '.join([ensembleVariance, ensembleVarianceRef]),
                                          output=self.tmpDir+fnFlag1+'_ensspread_vs_referror_tmp.nc')
            ensemblespreadscore = cdo.div(input=' '.join([ensembleVariance, ensembleVarianceRef]),
                                          output=self.outputDir+fnFlag1+'_ensspread_vs_referror.nc')
            ensemblespreadscore_ln_tmp = cdo.ln(input=cdo.div(input=' '.join([ensembleVariance, ensembleVarianceRef]),
                                                              output=self.tmpDir+fnFlag1+'_ensspread_vs_referror.nc'),
                                                output=self.tmpDir+fnFlag1+'_ensspread_vs_referror_ln.nc',
                                                options='-b 64')
            # We do this because ncview can't handle (-)Infinity values
            ensemblespreadscore_ln = cdo.setvals('-Infinity,1e20', input=ensemblespreadscore_ln_tmp,
                                                 output=self.outputDir+fnFlag1+'_ensspread_vs_referror_ln.nc')
            ...
            if self.maskMissingValues:
                if self.basic_output:
                    ensemblespreadscore = cdo.mul(input=' '.join([ensemblespreadscore, self.misvalMask]),
                                                  output=ensemblespreadscore+'miss_val')
                    crpss = [s for s in filesToPlot if 'ens-vs-ref_crpss.nc' in s][0]
                    ensemblespreadscore_ln = [s for s in filesToPlot if 'ensspread_vs_referror_ln' in s][0]
                else:
                    ensemblespreadscore = [s for s in filesToPlot if 'ensspread_vs_referror.nc' in s][0]
                    ensemblespreadscore_ln = [s for s in filesToPlot if 'ensspread_vs_referror_ln' in s][0]
                    crpss = [s for s in filesToPlot if 'ens-vs-ref_crpss.nc' in s][0]
                    crpssEC = [s for s in filesToPlot if 'ens-vs-clim_crpss.nc' in s][0]
                    crpssRC = [s for s in filesToPlot if 'ref-vs-clim_crpss.nc' in s][0]
            if self.fieldmean:
                try:
                ...
                if self.bootstrapSwitch:
                    b_ens_ref = self.bootstrap(crpsEns, crpsRef, 'ens-vs-ref_'+rangeStr, crpss, self.bootstrap_number,
                                               plot_range=[-0.5, 0.], colorbar='goddard')
                    if not self.basic_output:
                        b_ens_clim = self.bootstrap(crpsEns, crpsClim, 'ens-vs-clim_'+rangeStr, crpssEC,
                                                    self.bootstrap_number)
                        b_ref_clim = self.bootstrap(crpsRef, crpsClim, 'ref-vs-clim_'+rangeStr, crpssRC,
                                                    self.bootstrap_number)
                    self.bootstrapEnsembleSpreadscore(ensAnoms, anomalies, tempSmoothedObs, ensemblespreadscore,
                                                      self.bootstrap_number, ensemblespreadscore_ln)
            elif self.zonalmean:
                if self.bootstrapSwitch:
                    print 'Bootstrapping'
                    b_ens_ref = self.bootstrap(crpsEns, crpsRef, 'ens-vs-ref_'+rangeStr, crpss, self.bootstrap_number,
        ...
        if self.fieldmean:
            ...
            flag1 = flag1[4:]
            plot_list = [('ensspread_vs_referror_ln', 'Logarithmic Ensemble Spread Score', [-1,1]),
                         ('ens-vs-ref_crpss', 'Continous Ranked Probability Score', [-0.5,0])]
            if self.bootstrapSwitch:
                new_list = list()
    ...
    def removeConditionalBiasNew(self, hindcast, observations, ensembleMembers):
        ...
        #######################################
        bias = cdo.mul(input=corr+' ' + cdo.div(input=varObservations+' '+varHindcast),
                       output=self.tmpDir + 'conditional_biasNew.nc')
    ...
    def getReferenceStd(self, hindcast, observation):
        ...
        varSum = list()
        for year in self.decadals:
            varSum.append(cdo.sqr(input=cdo.sub(input=' '.join([hindcast[year], observation[year]]),
                                                output=hindcast[year]+self.getRandomStr()),
        ...
        return cdo.sqrt(input=cdo.divc((len(varSum)-2), input=rms, output=hindcast[year]+self.getRandomStr()+'divc'),
                        output=hindcast[year]+self.getRandomStr()+'refStd')
    ...
    def bootstrapEnsembleSpreadscore(self, ensAnoms, anomalies, tempSmoothedObs, spreadscore,
                                     bootstrap_number, spreadscore_ln):
        """
    ...
    def checkEnsembleError(self, ensList):
        ...
        for year in self.decadals:
            if len(ensList[year]) < 2:
                raise NotEnoughEnsemblemembersFound,\
                    'Not enough ensemblemembers found for %s %s %s %s %s' % (self.variable, self.project, self.model,
                                                                             self.project, year)
